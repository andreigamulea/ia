class ListaVegetalesController < ApplicationController
  ...
  def index
    ...
    @search_term = params[:search_term] # adaugă această linie

    if params[:search_type] == "eq"
      @lista_vegetales = ListaVegetale.where('specie ~* ? OR sinonime ~* ? OR parteutilizata ~* ?', "\\y#{params[:search_term]}\\y", "\\y#{params[:search_term]}\\y", "\\y#{params[:search_term]}\\y").page(params[:page]).per(10)
      @q = @lista_vegetales.ransack(params[:q])
    else
      @q = ListaVegetale.ransack({
        specie_cont: params[:search_term], 
        sinonime_cont: params[:search_term], 
        parteutilizata_cont: params[:search_term], 
        m: 'or'
      })
